...
def load_model(model_path, config_path, device='cpu'):
    ...
    # 체크포인트 로드 (PyTorch 2.6+ 기본 weights_only 변경 대응)
...
def predict_new_data(model, new_data_path, preprocessor_path, device='cpu',
    ...
    학습 시 저장한 `DataPreprocessor`로 transform한 뒤 모델에 입력한다.
    """
    ...
    X = pp.transform(df, date_columns=list(date_columns)).values.astype(np.float32)
