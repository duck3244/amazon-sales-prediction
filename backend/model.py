...
def get_model(model_type='basic', input_dim=10, **kwargs):
    ...
    공통 인자(`hidden_dims`, `dropout_rate`)를 모델별 시그니처에 맞게 변환한다.
    """
    ...
    if model_type == 'basic':
        ...
        return SalesPredictor(input_dim, hidden_dims=hidden_dims,
                              dropout_rate=dropout_rate, **kwargs)
    elif model_type == 'advanced':
        hidden_dim = hidden_dims[0] if hidden_dims else kwargs.pop('hidden_dim', 256)
        kwargs.pop('hidden_dim', None)
        return AdvancedSalesPredictor(input_dim, hidden_dim=hidden_dim,
                                      dropout_rate=dropout_rate, **kwargs)
    elif model_type == 'attention':
        hidden_dim = hidden_dims[0] if hidden_dims else kwargs.pop('hidden_dim', 256)
        kwargs.pop('hidden_dim', None)
        return AttentionSalesPredictor(input_dim, hidden_dim=hidden_dim,
                                       dropout_rate=dropout_rate, **kwargs)
    else:
